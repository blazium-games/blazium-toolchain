...
- Affected command (`ps1 setup`, `ps2 build`, `n64 run`, `interdvd iso`, …)
- Host OS
- Whether the prefix cache or a fetched zip is involved
- A minimal reproduction using only this CLI and `--prefix` (no editor checkout)
- The stderr `error` / `---` block from the CLI (command, host, version, prefix)
