...
func (t *Tool) Build(ctx context.Context, opts platforms.BuildOptions) error {
	...
	if !compileReady(env) {
		return report.Missing("ps2 compile tools not ready", "blazium-toolchain ps2 setup --profile compile")
	}
	...
	if src == "" || opts.Out == "" {
		return report.Usage("build requires --out FILE.elf")
	}
	...
	if fileExists(filepath.Join(src, "Makefile")) || fileExists(filepath.Join(src, "Makefile.sample")) {
		...
		return report.Missing("no Makefile or CMakeLists.txt in "+src, "")
	}
	...
	if err := checkELFBudget(opts.Out); err != nil {
		return err
	}
	if opts.Stdout != nil {
...
func checkELFBudget(path string) error {
	...
	if s.ElfTextMax > 0 {
		text, err := TextSectionSize(path)
		if err != nil {
			return err
		}
		if int64(text) > s.ElfTextMax {
...
func (t *Tool) buildMake(ctx context.Context, src, buildDir string, extraPath []string, extraEnv map[string]string, opts platforms.BuildOptions) error {
	...
	if makeProg == "" {
		return report.Missing("make not found (needed to build the ps2sdk guest)", "install make and put it on PATH")
	}
	makefile := "Makefile"
...
func (t *Tool) buildCMake(ctx context.Context, src, buildDir string, extraPath []string, extraEnv map[string]string, opts platforms.BuildOptions) error {
	...
	if cmake == "" {
		return report.Missing("cmake not found", "install CMake and put it on PATH")
	}
	...
	if err := t.runEnv(ctx, cmake, cfg, extraPath, extraEnv, opts.Stdout, opts.Stderr); err != nil {
		return report.Fail("cmake configure: "+err.Error(), "blazium-toolchain ps2 setup --profile compile")
	}
...
func gnuMkdir() string {
	...
	for _, p := range settings.Current().PS2.MkdirPath {
		if fileExists(p) {
			return p
		}
	}
...
func resolveSample(env map[string]string, sample string) (string, error) {
	if sample != "cube" {
		return "", report.Usage("--sample must be cube")
	}
	sdk := env["PS2SDK"]
	...
	return "", report.Missing("cube sample not in PS2SDK ("+p+")", "blazium-toolchain ps2 setup --profile compile")
}
...
func findMipsELF(buildDir, src string) (string, error) {
	...
	return "", report.Fail(fmt.Sprintf("no MIPS ELF in %s or %s after build", buildDir, src), "check the guest CMakeLists / Makefile")
}
