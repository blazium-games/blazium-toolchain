...
func plantCompileTools(t *testing.T, prefix string) {
	t.Helper()
	inst := filepath.Join(prefix, "n64", "inst")
...
func TestStageCookPackDirStagesExtras(t *testing.T) {
	...
	prtn := []byte{'P', 'R', 'T', 'N', 1, 0, 0, 0}
	if err := os.WriteFile(filepath.Join(src, "PRTN00.bin"), prtn, 0o644); err != nil {
		t.Fatal(err)
	}
	if err := stageCookEmbed(dest, opts); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(dest, "filesystem", "PRTN00.bin")); err != nil {
		t.Fatalf("missing staged PRTN00.bin: %v", err)
	}
}
...
func (memFetch) FetchZip(ctx context.Context, url, sha, destDir string, log io.Writer) error {
	return srcFetch{}.FetchZip(ctx, url, sha, destDir, log)
}
...
func TestRunSkipsMissingSingleEmu(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "skip") || !strings.Contains(buf.String(), "project64") {
...
func TestRunSkipsPj64WithoutVideoPlugin(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "skip") || !strings.Contains(buf.String(), "project64") || !strings.Contains(buf.String(), "Parallel-RDP") {
...
func TestRunSkipsPj64Nopak(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "skip") || !strings.Contains(out, "project64") || !strings.Contains(out, "4 MiB no-pak") {
...
func TestRunBothFailsWhenOnlyAresRan(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "both Ares and Project64 are installed") {
		t.Fatalf("got %v\n%s", err, buf.String())
	}
	if !emuBaseHas(fake.names, "ares") || !emuBaseHas(fake.names, "project64") {
		t.Fatalf("must invoke both, got %v", fake.names)
...
func TestResolveSampleT3dQuad(t *testing.T) {
	...
	chdir(t, root)
	got, err := resolveSample(root, "t3dquad")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, alias := range []string{"00_quad", "tiny3d"} {
		g, err := resolveSample(root, alias)
		if err != nil {
...
func TestResolveSampleOvlDemo(t *testing.T) {
	...
	chdir(t, root)
	got, err := resolveSample(root, "ovldemo")
	if err != nil {
		t.Fatal(err)
	}
...
func TestOfficialToolchainURLLinux(t *testing.T) {
	win := officialToolchainURLFor("windows", "amd64")
	if !strings.Contains(win, "gcc-toolchain-mips64-win64.zip") {
		t.Fatalf("windows toolchain: %s", win)
	}
	linux := officialToolchainURLFor("linux", "amd64")
	if !strings.HasSuffix(linux, "gcc-toolchain-mips64-x86_64.deb") {
		t.Fatalf("linux toolchain must be the official .deb (no sibling tree): %s", linux)
	}
	arm := officialToolchainURLFor("linux", "arm64")
	if !strings.HasSuffix(arm, "gcc-toolchain-mips64-aarch64.deb") {
		t.Fatalf("linux arm64 toolchain: %s", arm)
	}
	if officialToolchainURLFor("darwin", "arm64") != "" {
		t.Fatal("darwin must not get an N64 toolchain URL")
	}
}

func TestOfficialSourceURLs(t *testing.T) {
	lib := officialLibdragonSrcURL()
	if !strings.Contains(lib, "DragonMinded/libdragon") || !strings.HasSuffix(lib, "preview.zip") {
		t.Fatalf("libdragon source: %s", lib)
	}
	t3d := officialTiny3dSrcURL()
	if !strings.Contains(t3d, "HailToDodongo/tiny3d") || !strings.Contains(t3d, ".zip") {
		t.Fatalf("tiny3d source: %s", t3d)
	}
}

func TestFindSrcRootUnwrapsGithubZip(t *testing.T) {
	dir := t.TempDir()
	nested := filepath.Join(dir, "libdragon-preview")
	if err := os.MkdirAll(nested, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(nested, "n64.mk"), []byte("# n64\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	got := findSrcRoot(dir, "n64.mk")
	if filepath.Clean(got) != filepath.Clean(nested) {
		t.Fatalf("got %q want %q", got, nested)
	}
}

func TestResolveSampleFromCachedSrc(t *testing.T) {
	prefix := t.TempDir()
	lib := filepath.Join(prefix, "n64", "src", "libdragon", "libdragon-preview")
	hello := filepath.Join(lib, "examples", "helloworld")
	...
	if err := os.WriteFile(filepath.Join(hello, "Makefile"), []byte("all:\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	t3d := filepath.Join(prefix, "n64", "src", "tiny3d", "tiny3d-master")
	quad := filepath.Join(t3d, "examples", "00_quad")
	if err := os.MkdirAll(quad, 0o755); err != nil {
		t.Fatal(err)
	}
	...
	got, err := resolveSample(prefix, "helloworld")
	if err != nil {
		t.Fatal(err)
	}
	if filepath.Clean(got) != filepath.Clean(hello) {
		t.Fatalf("helloworld -> %q want %q", got, hello)
	}
	got, err = resolveSample(prefix, "t3dquad")
	if err != nil {
		t.Fatal(err)
	}
	if filepath.Clean(got) != filepath.Clean(quad) {
		t.Fatalf("t3dquad -> %q want %q", got, quad)
	}
}

type srcFetch struct{}

func (srcFetch) FetchZip(_ context.Context, url, _, destDir string, _ io.Writer) error {
	if err := os.MkdirAll(destDir, 0o755); err != nil {
		return err
	}
	if strings.Contains(url, "tiny3d") {
		root := filepath.Join(destDir, "tiny3d-master")
		if err := os.MkdirAll(filepath.Join(root, "examples", "00_quad"), 0o755); err != nil {
			return err
		}
		if err := os.WriteFile(filepath.Join(root, "t3d.mk"), []byte("# t3d\n"), 0o644); err != nil {
			return err
		}
		return os.WriteFile(filepath.Join(root, "examples", "00_quad", "Makefile"), []byte("all:\n"), 0o644)
	}
	if strings.Contains(url, "libdragon") && strings.Contains(url, "preview") {
		root := filepath.Join(destDir, "libdragon-preview")
		if err := os.MkdirAll(filepath.Join(root, "examples", "helloworld"), 0o755); err != nil {
			return err
		}
		if err := os.WriteFile(filepath.Join(root, "n64.mk"), []byte("# n64\n"), 0o644); err != nil {
			return err
		}
		return os.WriteFile(filepath.Join(root, "examples", "helloworld", "Makefile"), []byte("all:\n"), 0o644)
	}
	name := "ares"
	if runtime.GOOS == "windows" {
		name += ".exe"
...
func TestSetupFetchesLibdragonAndTiny3dSources(t *testing.T) {
	dir := t.TempDir()
	plantCompileTools(t, dir)
	t.Setenv("N64_INST", "")
	t.Setenv("N64_GCC", "")
	tool := &Tool{Fetcher: srcFetch{}}
	if err := tool.ensureLibdragonSrc(context.Background(), dir, nil); err != nil {
		t.Fatal(err)
	}
	if err := tool.ensureTiny3dSrc(context.Background(), dir, nil); err != nil {
		t.Fatal(err)
	}
	if libdragonRoot(dir) == "" {
		t.Fatal("setup must cache libdragon preview (no sibling tree)")
	}
	if tiny3dRoot(dir) == "" {
		t.Fatal("setup must cache Tiny3D (no sibling tree)")
	}
	got, err := resolveSample(dir, "helloworld")
	if err != nil {
		t.Fatal(err)
	}
	if !fileExists(filepath.Join(got, "Makefile")) {
		t.Fatalf("cached helloworld missing Makefile: %s", got)
	}
}
