...
func (t *Tool) Setup(ctx context.Context, opts platforms.SetupOptions) error {
	...
	if opts.Offline && !encodeReady(env) {
		return report.Offline("interdvd ffmpeg/ffprobe missing under prefix", "drop --offline or run blazium-toolchain interdvd setup")
	}
	if !encodeReady(env) {
	...
	report.Setup(opts.Stdout, ID, "encode", cache.PlatformDir(opts.Prefix, ID), env)
	return nil
}
...
func (t *Tool) Build(_ context.Context, _ platforms.BuildOptions) error {
	return report.Usage("interdvd has no build; author VIDEO_TS in the editor")
}

func (t *Tool) Run(_ context.Context, _ platforms.RunOptions) error {
	return report.Usage("interdvd has no run; burn or play the ISO externally")
}
...
func (t *Tool) ISO(_ context.Context, opts platforms.ISOOptions) error {
	...
	report.Linef(opts.Stdout, report.Wrote, "%s  %d sectors  volume %s", resolved.Out, res.TotalSectors, res.VolumeID)
	return nil
}
