...
func (t *Tool) ensureFFmpeg(ctx context.Context, prefix string, log io.Writer) error {
	...
	if len(assets) == 0 {
		return fmt.Errorf("%w: no interdvd ffmpeg pin for %s", platforms.ErrMissingTool, runtime.GOOS)
	}
...
func (t *Tool) RunTool(ctx context.Context, name string, args []string, opts platforms.CommonOptions) error {
	...
	case "ffprobe":
		bin = env["FFPROBE"]
	default:
		return fmt.Errorf("%w: interdvd %s", platforms.ErrUsage, name)
	}
	if bin == "" || !fileExists(bin) {
		return fmt.Errorf("%w: run blazium-toolchain interdvd setup first", platforms.ErrMissingTool)
	}
