...
func (h HTTP) client() *http.Client {
	...
	return &http.Client{Timeout: settings.Current().Download()}
}
...
func (h HTTP) download(ctx context.Context, url, dest, wantSHA string, log io.Writer) error {
	if st, err := os.Stat(dest); err == nil && st.Size() > 0 {
		if wantSHA == "" || hashFile(dest) == strings.ToLower(wantSHA) {
			if log != nil {
				report.Line(log, report.Reusing, dest)
			}
			return nil
		}
		_ = os.Remove(dest)
	...
	if resp.StatusCode != http.StatusOK {
		return report.Fail(fmt.Sprintf("download %s: HTTP %s", url, resp.Status), "check the URL in blazium-toolchain.yml")
	}

	max := settings.Current().DownloadMax()
	if max > 0 && resp.ContentLength > max {
		return report.Fail(fmt.Sprintf("download %s is %d bytes (max %d)", url, resp.ContentLength, max), "raise download_max_bytes in blazium-toolchain.yml")
	}
	...
	if log != nil {
		report.Line(log, report.Fetching, url)
	}
	limit := resp.Body
	if max > 0 {
		limit = io.NopCloser(io.LimitReader(resp.Body, max+1))
	}
	n, err := io.Copy(w, limit)
	if err != nil {
		f.Close()
		_ = os.Remove(tmp)
		return err
	...
	if max > 0 && n > max {
		f.Close()
		_ = os.Remove(tmp)
		return report.Fail(fmt.Sprintf("download %s exceeded %d bytes", url, max), "raise download_max_bytes in blazium-toolchain.yml")
	}
	...
	if wantSHA != "" && got != strings.ToLower(wantSHA) {
		_ = os.Remove(tmp)
		return report.Fail(fmt.Sprintf("sha256 mismatch for %s: got %s want %s", url, got, wantSHA), "delete the downloads cache or update pins.json")
	}
...
func ExtractArchive(archivePath, destDir string) error {
	...
	default:
		return report.Fail("unsupported archive "+filepath.Base(archivePath), "need zip, tar.gz, tar.xz, tar.zst, or deb")
	}
}
...
func extractTarXZ(archivePath, destDir string) error {
	...
	if err != nil {
		return report.Fail("xz "+filepath.Base(archivePath)+": "+err.Error(), "")
	}
	return readTarToDir(xr, destDir)
}
...
func materializeTarLinks(destDir string, links []tarLink) error {
	for _, l := range links {
		...
		if err := safepath.Under(destDir, src); err != nil {
			return report.Fail("refusing tar link "+l.linkname, "")
		}
		if err := safepath.Under(destDir, l.dest); err != nil {
			return report.Fail("refusing tar link dest "+l.dest, "")
		}
...
func extractTarSystem(archivePath, destDir string) error {
	...
	if err != nil {
		return report.Fail(fmt.Sprintf("tar extract %s: %s", filepath.Base(archivePath), strings.TrimSpace(string(out))), "")
	}
	return nil
}

// ExtractDeb unpacks a Debian .deb (ar + data.tar.*) into destDir without dpkg.
func ExtractDeb(archivePath, destDir string) error {
	raw, err := os.ReadFile(archivePath)
	if err != nil {
		return err
	}
	data, name, err := arMember(raw, "data.tar")
	if err != nil {
		return fmt.Errorf("deb %s: %w", filepath.Base(archivePath), err)
	}
	suffix := ".tar"
	switch {
	...
	tmp, err := os.CreateTemp("", "blazium-deb-data-*"+suffix)
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
	...
	return extractTar(tmpName, destDir)
}

func arMember(archive []byte, namePrefix string) ([]byte, string, error) {
	if !bytes.HasPrefix(archive, []byte("!<arch>\n")) {
		return nil, "", fmt.Errorf("not a unix ar archive")
	}
	off := 8
	for off+60 <= len(archive) {
		hdr := archive[off : off+60]
		off += 60
		name := strings.TrimSpace(string(bytes.TrimRight(hdr[0:16], "/")))
		sizeStr := strings.TrimSpace(string(hdr[48:58]))
		size, err := strconv.ParseInt(sizeStr, 10, 64)
		if err != nil || size < 0 {
			return nil, "", fmt.Errorf("bad ar size %q", sizeStr)
		}
		if off+int(size) > len(archive) {
			return nil, "", fmt.Errorf("truncated ar member %s", name)
		}
		body := archive[off : off+int(size)]
		off += int(size)
		if off%2 == 1 {
			off++
		}
	...
	return nil, "", fmt.Errorf("ar member %s* not found", namePrefix)
}
