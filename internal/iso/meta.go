...
func LoadMeta(path string) (DiscMeta, error) {
	...
	if err != nil {
		return DiscMeta{}, report.Fail("iso: read meta: "+err.Error(), "pass --meta to a disc.interdvd.json")
	}
	var m DiscMeta
	if err := json.Unmarshal(b, &m); err != nil {
		return DiscMeta{}, report.Fail("iso: parse meta: "+err.Error(), "fix disc.interdvd.json")
	}
	if m.Schema != "" && m.Schema != SchemaV1 {
		return DiscMeta{}, report.Fail(fmt.Sprintf("iso: unsupported meta schema %q", m.Schema), "use blazium.interdvd.meta/v1")
	}
	m.Schema = SchemaV1
...
func WriteMeta(path string, m DiscMeta) error {
	...
	if err := enc.Encode(m); err != nil {
		return report.Fail("iso: encode meta: "+err.Error(), "")
	}
...
func (m DiscMeta) Resolve() (DiscMeta, error) {
	...
	m.Volume = VolumeOr(m.Volume, settings.Current().InterDVD.Volume)
	if m.Title == "" {
		m.Title = m.Volume
	}
	d := settings.Current().InterDVD
	if m.Preparer == "" {
		m.Preparer = d.Preparer
	}
	if m.Application == "" {
	...
	if m.MenuLanguage == "" {
		m.MenuLanguage = d.MenuLanguage
	}
	if m.AudioLanguage == "" {
		m.AudioLanguage = d.AudioLanguage
	}
	if m.SubtitleLanguage == "" {
		m.SubtitleLanguage = d.SubtitleLanguage
	}
	if m.Disc == 0 {
		m.Disc = 1
	}
	...
	if m.Disc < 1 || m.Discs < 1 || m.Disc > m.Discs {
		return m, report.Fail(fmt.Sprintf("iso: disc %d of %d is invalid", m.Disc, m.Discs), "set disc and discs to 1 or higher")
	}
	if m.RegionMask != 0 && (m.RegionMask < 1 || m.RegionMask > 255) {
		return m, report.Fail("iso: region_mask must be 1-255 when set", "this field is stored in disc.interdvd.json only")
	}
	if m.ParentalLevel != 0 && (m.ParentalLevel < 1 || m.ParentalLevel > 8) {
		return m, report.Fail("iso: parental_level must be 1-8 when set", "this field is stored in disc.interdvd.json only")
	}
	if m.Created != "" {
		if _, err := time.Parse(time.RFC3339, m.Created); err != nil {
			return m, report.Fail("iso: created must be RFC3339: "+err.Error(), "")
		}
	}
